...
| Variable references | `$NAME`, `${NAME}`, `$$`, `$?`, `$_`, `$-`, positional `$N` and `${N}`, argument count `$#` and `${#}`, `${NAME-word}`, `${NAME:-word}`, `${NAME+word}`, `${NAME:+word}`, `${NAME:=word}`, `${NAME:?word}`, `${#NAME}`, pattern removal with `#`, `##`, `%`, and `%%`, and ASCII case forms with `^`, `^^`, `,`, and `,,`; see [ShellExpressions.md](ShellExpressions.md) |
...
| Logging | `LOGFILE`, `LOG`, `VERBOSE`, `LOGABSTRACT`, and `LOGDETAIL=values`; text and JSON formats include session boundaries, recipe decisions, native header operations, and delivery outcomes |
...
| Shell-style assignments, including quoting, escapes, unsetting, field splitting, and parameter forms | procmail-rs supports the bounded expression language documented in [ShellExpressions.md](ShellExpressions.md), including bare-name removal and several parameter forms beyond original procmail. | Removed and explicitly empty values remain distinct across blocks and runtime rc files. Field splitting of command output remains absent. Unsupported forms, extra words, and unterminated quotes are rejected instead of being assigned a different meaning. |
| Repeatable `-a argument`, numbered `$1`, `$2`, ... parameters, `$#`, special `"$@"` program arguments, and `SHIFT` | Supported through 256 arguments, with per-value and aggregate byte ceilings. Missing numbered parameters expand to empty. A standalone quoted `"$@"` passes the current window as distinct external-program arguments; only the rightmost occurrence expands. A positive decimal `SHIFT` advances the remaining argument window and caps an excessive shift at `$#`. | `$*`, unquoted `$@`, and `$@` embedded in another word remain unsupported. Positional values and shifts cross runtime include and switch boundaries but are not exported as numeric child-environment names. Copy branches shift their private runtime state. Unlike original procmail, zero, negative, and malformed `SHIFT` values are rejected instead of being ignored. |
| Scalar special parameters `$$`, `$?`, `$_`, and `$-` | Supported as the process id, last external-command status, current rc-file name, and `LASTFOLDER`. | A normal child exit preserves its exact code. Signal termination and timeout use `1` when no numeric exit code is available. An asynchronous pipe records successful launch as `0`; its later completion does not change following expressions. The special values remain private runtime state rather than child-environment entries. |
...
| --- | --- | --- |
| Supported or intentionally narrowed | `HOME`, `LOGNAME`, `PATH`, `SHELL`, `SHELLFLAGS`, `MAILDIR`, `LOGFILE`, `LOG`, `VERBOSE`, `LOGABSTRACT`, `LOCKFILE`, `LOCKEXT`, `LOCKSLEEP`, `LOCKTIMEOUT`, `TIMEOUT`, `HOST`, `UMASK`, `TRAP`, `EXITCODE`, `LASTFOLDER`, `MATCH`, `INCLUDERC`, `SWITCHRC`, `PROCMAIL_VERSION`, and `LINEBUF` | Exact restrictions are recorded in this document and in the limits documentation. `MATCH1`, `MATCH2`, and later numbered captures are procmail-rs additions. |
| Explicitly rejected | `DEFAULT`, `ORGMAIL`, `COMSAT`, `DELIVERED`, `DROPPRIVS`, `MSGPREFIX`, `NORESRETRY`, `PROCMAIL_OVERFLOW`, `SHELLMETAS`, `SUSPEND`, `SENDMAIL`, `SENDMAILFLAGS`, and `LIMIT_RC_SIZE` | These names cannot accidentally act as ordinary variables. Implementing `DROPPRIVS` is outside project scope. `LIMIT_RC_SIZE` cannot safely configure the read which has already consumed its own assignment. |
...
| Non-Linux Unix Maildir publication | Uses a named file in `tmp`. | Uses exclusive named creation in `tmp`, no-replace hard-link publication in `new`, and inode checks before cleanup. It additionally requires the Maildir directories to be owned by the current uid and not writable by group or other users. The portable pathname-based implementation retains a race between each check and operation; Linux uses the stronger unnamed-file path. |
...
| Unsupported reserved variables | Variables such as `DEFAULT`, `ORGMAIL`, `COMSAT`, `DELIVERED`, `DROPPRIVS`, `MSGPREFIX`, `NORESRETRY`, `PROCMAIL_OVERFLOW`, `SHELLMETAS`, `SUSPEND`, `SENDMAIL`, and `SENDMAILFLAGS` retain their original special meanings. | Rejects these names and the project-reserved `LIMIT_RC_SIZE` explicitly in assignments, `--set`, and expansion references. Unknown names remain ordinary user variables. |
| `LOG=value` | Appends the assigned value to `LOGFILE` without adding a newline. | Text output preserves this behavior for values through 256 bytes. JSON output emits a structured `log` record. The fixed bound prevents one assignment from bypassing trace record limits. |
| `LOGABSTRACT` | Defaults to a final abstract containing `From`, `Subject`, destination, and message size; `no` suppresses it and `all` logs every successful delivery. | Defaults to `no`. `yes` reports the final successful delivery and `all` reports every successful delivery. Metadata mode includes only the destination kind and recipe line; `LOGDETAIL=values` may additionally include the bounded destination path. Header values and message bodies are never included, and message size is not yet reported. |
