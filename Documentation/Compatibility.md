...
| Unsupported reserved variables | Variables such as `DEFAULT`, `ORGMAIL`, `COMSAT`, `DELIVERED`, `LOCKEXT`, `LOG`, `LOGABSTRACT`, `MSGPREFIX`, `NORESRETRY`, `PROCMAIL_OVERFLOW`, `PROCMAIL_VERSION`, `SHELLMETAS`, `SUSPEND`, `SENDMAIL`, `SENDMAILFLAGS`, and `SHIFT` retain their original special meanings. | Rejects these names explicitly in assignments, `--set`, and expansion references. Unknown names remain ordinary user variables. |
