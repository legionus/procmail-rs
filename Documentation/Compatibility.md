...
| Repeatable `-a argument`, numbered `$1`, `$2`, ... parameters, and `$#` | Supported through 256 arguments, with per-value and aggregate byte ceilings. Missing numbered parameters expand to empty. | `$@`, `$*`, and `SHIFT` remain explicitly unsupported. Positional values are available across runtime include and switch boundaries but are not exported as numeric child-environment names. |
