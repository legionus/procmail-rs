...
: Selects a bounded delivery summary. `no` disables summaries and is the
  default. `yes` writes one summary for the final successful delivery. `all`
  writes one for every successful delivery, including copy recipes. Metadata
  mode reports only the destination kind and recipe line. With
  `LOGDETAIL=values`, the bounded destination path is also shown. Unlike
  original procmail, summaries never contain `From`, `Subject`, or message
  body data, and currently omit message size.

`LOG`

: Appends its expanded value to the active trace output at the point where the
  assignment executes. It does not enable verbose diagnostics and does not add
  a newline. The value is limited to 256 bytes. In text format the value is
  appended directly; JSON format emits a `log` object so the output remains a
  valid record stream.

  ```procmail
  LOG="entering list rules for $LOGNAME"
  ```
...
`DROPPRIVS`, `MSGPREFIX`, `NORESRETRY`,
...
escaped variable value prefixes. `LOGABSTRACT=yes` writes the final delivery
summary, while `LOGABSTRACT=all` writes one for each successful delivery.
